...
pub struct WhisperModelParams {
    pub use_gpu: bool,
    /// Enable Flash Attention (faster on GPU, incompatible with DTW timestamps).
    pub flash_attn: bool,
}
...
pub struct WhisperInferenceParams {
    ...
    /// Use greedy decoding with the given `best_of` value instead of beam search.
    /// This is typically much faster and is a better default for latency-sensitive
    /// models such as Whisper Turbo.
    pub greedy_best_of: Option<i32>,

    /// Number of CPU threads to use during inference. If `None`, whisper.cpp
    /// picks its own default.
    pub n_threads: Option<i32>,

    /// Enables whisper.cpp debug logging.
    pub debug_mode: bool,

    /// Skip timestamp computation entirely. Faster when only text output is needed.
...
impl TranscriptionEngine for WhisperEngine {
    ...
    fn transcribe_samples(
        ...
        let strategy = match whisper_params.greedy_best_of {
            Some(best_of) => SamplingStrategy::Greedy { best_of },
            None => SamplingStrategy::BeamSearch {
                beam_size: 3,
                patience: -1.0,
            },
        };
