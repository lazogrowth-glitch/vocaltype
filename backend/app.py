...
def init_db():
    ...
    # One row per device that has ever registered an account.
    # device_id is the UUID generated by the desktop app on first launch.
    db.execute(
        """
...
def device_is_registered(device_id: str) -> bool:
    """Return True if this device UUID already has a registered account."""
    if not device_id:
        return False
    db = get_db()
    try:
...
def register_device(device_id: str, user_id: int) -> None:
    """Associate a device UUID with a user after successful registration."""
    if not device_id:
        return
    db = get_db()
    try:
...
def send_reset_email(to_email: str, code: str) -> None:
    """Send a password-reset email if SMTP env vars are configured."""
