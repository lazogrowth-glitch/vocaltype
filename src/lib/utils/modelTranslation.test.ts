...
describe("getTranslatedModelName", () => {
  it("returns translated name when translation key resolves to a non-empty string", () => {
    ...
    expect(getTranslatedModelName(makeModel(), t as never)).toBe(
      "Petit Whisper",
    );
    expect(t).toHaveBeenCalledWith("onboarding.models.whisper-small.name", {
      defaultValue: "",
    });
  });
  ...
  it("uses model.id in the translation key", () => {
    ...
    expect(t).toHaveBeenCalledWith("onboarding.models.parakeet-v3.name", {
      defaultValue: "",
    });
  });
});
...
describe("getTranslatedModelDescription", () => {
  it("returns translated description when translation key resolves", () => {
    ...
    expect(getTranslatedModelDescription(makeModel(), t as never)).toBe(
      "Un modèle rapide",
    );
    expect(t).toHaveBeenCalledWith(
      "onboarding.models.whisper-small.description",
      {
        defaultValue: "",
      },
    );
  ...
  it("does not call the per-model description key for custom models", () => {
    ...
    const keys = (t as ReturnType<typeof vi.fn>).mock.calls.map(
      (c: unknown[]) => c[0],
    );
  ...
  it("uses model.id in the description translation key", () => {
    const t = vi.fn().mockReturnValue("");
    getTranslatedModelDescription(
      makeModel({ id: "parakeet-tdt-0.6b-v3" }),
      t as never,
    );
    expect(t).toHaveBeenCalledWith(
